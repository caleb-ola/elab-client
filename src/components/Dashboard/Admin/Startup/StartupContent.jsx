...
function StartupContent() {
  ...
  const RenderData = () => {
    axios.get(`${process.env.REACT_APP_BASE_URL}/api/v1/startups`).then(
      (response) => {
        ...
        if (response.data.data.length === 0) {
          setBody(
            <DashboardEmpty header="No Startup" subtext="Startups are yet to be added, proceed to add a new startup." />,
          );
        } else {
          setBody(
            <>
                          ...
                          (item, index) => (
                            <tr key={item.id}>
                              ...
                              <td className="text-end">
                                <Link to={`/dashboard/admin/edit-startup/${item.slug}`}>
                                  <span
                                      ...
                                      <div className="modal-dialog modal-dialog-centered">
                                        <div className="modal-content">
                                          ...
                                          <div className="modal-footer  border-0">
                                            ...
                                            // eslint-disable-next-line no-use-before-define
                                              onClick={() => DeleteStartup(item.slug)}
                                              data-bs-dismiss="modal"
                                            >
                                              Confirm Delete
                                            </button>
  ...
  const DeleteStartup = (slug) => {
    axios.delete(`${process.env.REACT_APP_BASE_URL}/api/v1/startups/${slug}`, {
      headers: {
    ...
    <div className="content px-4">
      ...
      <div className="row content__header align-items-center mb-5">
        <div className="col-md-6 p-0 text-center text-md-start">
          <h4 className="m-0">Startup Foundry</h4>
        </div>
        <div className="col-md-6 p-0 text-center text-md-end">
          <Link to="/dashboard/admin/add-startup" className="link px-4 py-2 text-decoration-none fw-bold">Add a Startup</Link>
        </div>
      </div>
