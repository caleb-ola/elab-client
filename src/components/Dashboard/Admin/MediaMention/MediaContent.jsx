...
function MediaContent() {
  ...
  const RenderData = () => {
    axios.get(`${process.env.REACT_APP_BASE_URL}/api/v1/mentions`).then(
      (response) => {
        ...
        if (response.data.data.length === 0) {
          setBody(
            <DashboardEmpty header="No Media Mention" subtext="Media mentions are yet to be added." />,
          );
        } else {
          setBody(
            <>
              <table className="w-100 text-start px-5 mx-2 mx-lg-5">
                <thead>
                  <tr className="fw-bold">
                    ...
                    <th className="pe-4">Logo</th>
                    <th className="pe-4">Media name</th>
                    <th>Media url</th>
                    <th>Created On</th>
                  </tr>
                </thead>
                          ...
                          (item, index) => (
                            <tr key={item.id}>
                              ...
                              <td className="text-end">
                                <Link to={`/dashboard/admin/edit-media/${item.id}`}>
                                  <span
  ...
  const DeleteMention = (id) => {
    axios.delete(`${process.env.REACT_APP_BASE_URL}/api/v1/mentions/${id}`, {
      headers: {
      ...
      // console.log(response);
      toast.success(
        response?.data?.message || 'You successfully deleted a project.',
        {
          position: 'top-right',
          autoClose: 15000,
          hideProgressBar: true,
    ...
    <div className="content px-4">
      ...
      <div className="row content__header align-items-center mb-5">
        <div className="col-md-6 p-0 text-center text-md-start">
          <h4 className="m-0">Media Mention</h4>
        </div>
        <div className="col-md-6 p-0 text-center text-md-end">
          <Link to="/dashboard/admin/add-media" className="link px-4 py-2 text-decoration-none fw-bold">Add a Media Mention</Link>
        </div>
      </div>
