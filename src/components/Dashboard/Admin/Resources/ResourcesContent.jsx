...
function ResourcesContent() {
  ...
  const RenderData = () => {
    axios.get(`${process.env.REACT_APP_BASE_URL}/api/v1/resources`).then((response) => {
      ...
      if (response.data.data.length === 0) {
        setBody(
          <DashboardEmpty header="No Resource Material" subtext="Resources are yet to be added, proceed to add a new resource material." />,
        );
      } else {
        setBody(
          <>
                    ...
                    response.data.data.slice(paginate.start, paginate.end).map((item, index) => (
                      <tr key={item.id}>
                        ...
                        <td className="text-end">
                          <Link to={`/dashboard/admin/edit-resources/${item.slug}`}>
                            <span
                                  ...
                                  <div className="modal-content">
                                    ...
                                    <div className="modal-footer  border-0">
                                      ...
                                      <button
                                        ...
                                        className="btn btn-danger shadow-none"
                                        // eslint-disable-next-line no-use-before-define
                                        onClick={() => DeleteResource(item.slug)}
                                        data-bs-dismiss="modal"
                                      >
                                        Confirm Delete
                                      </button>
  ...
  const DeleteResource = (slug) => {
    axios.delete(`${process.env.REACT_APP_BASE_URL}/api/v1/resources/${slug}`, {
      ...
      // console.log(response);
      toast.success(
        response?.data?.message || 'You successfully deleted a resource.',
        {
          position: 'top-right',
          autoClose: 15000,
          hideProgressBar: true,
    ...
    <div className="content px-4">
      ...
      <div className="row content__header align-items-center mb-5">
        ...
        <div className="col-md-6 p-0 text-center text-md-end">
          <Link to="/dashboard/admin/add-resources" className="link px-4 py-2 text-decoration-none fw-bold">Add a Resource</Link>
        </div>
      </div>
